...
/// @brief Interface for Non-volatile storage, can be implemented on top of flash or other memory types.
///
...
public:
    ...
    enum Result {
        ...
        /// The storage was not in READY state
        NOT_READY = -1,
    ...
    /// @brief Current state of the storage.
    ...
    /// @brief Clear all elements in the non-volatile storage. Calling mount() is not necessary after clear.
    ...
    /// @param result number of bytes actually read or negative on error (see enum Result)
    /// @return use co_await on return value to await completion
    ...
    /// @param result number of bytes written or negative on error (see enum Result)
    /// @return use co_await on return value to await completion
